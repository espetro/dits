...
Voice interviews run over the server's own WebSocket voice loop; no extra
service is needed beyond the STT/TTS endpoints above. For fully local real voice,
`scripts/local-voice-stack.sh` brings up a parakeet STT (:9003) and pocket-tts
(:9004, shimmed at :9005) stack via process-compose.
...
- **Port already in use**: the server binds `server.port` (default `3000`), the
  mock provider `9000`. Override the server port with
...
- **Voice fails in the browser**: mic permissions are required for voice; the
  interview still works over the "type" text path. Check the `stt`/`tts`
  endpoints with `--check`.
