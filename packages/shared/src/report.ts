...
export function buildReportPrompt(ctx: ReportPromptContext): string {
  ...
  return [
    ...
    "",
    "Produce a report as a single JSON object with exactly these keys:",
    "{",
    '  "session_id": "<the session_id above>",',
    '  "overall_score": <0-10>,',
    '  "coverage_pct": <0-100, how much of the plan the interview covered>,',
    '  "competencies": [',
    '    {"name": "<skill>", "score": <0-10>, "evidence": [',
    '      {"quote": "<verbatim transcript text>", "turn_seq": <seq>,',
    '       "verdict": "worked" | "improve" | "drop"}',
    "    ]},",
    "  ],",
    '  "model_answers": [',
    '    {"question_id": "<uuid>", "question_text": "...", "answer": "..."}',
    "  ],",
    '  "generated_at": "<current ISO 8601 timestamp>"',
    "}",
    "One competency entry per distinct skill demonstrated; every evidence",
    "entry cites a transcript turn by its seq number. model_answers lists",
    "what a strong answer looks like for any question the candidate",
    "struggled with (empty array if none).",
    "Return ONLY the JSON object itself: no wrapping key, no markdown",
