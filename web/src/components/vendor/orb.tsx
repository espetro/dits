...
import { Canvas, useFrame, useStore, useThree, advance as r3fAdvance } from "@react-three/fiber";
...
function Scene({
  ...
  // di fix: under React StrictMode the Canvas effect cleanup runs once on
  // mount (unmountComponentAtNode -> forceContextLoss after a 500ms delay).
  // The surviving root's internal.active ends up false while r3f's module
  // level `running` flag stays true, so the global loop never restarts and
  // invalidate() is a no-op: the orb renders one frame then freezes. Work
  // around it with frameloop "never" + our own rAF calling advance(), which
  // updates the scene and renders without touching the global loop.
  useEffect(() => {
    setFrameloop("never");
    rootStore.setState((s) =>
      s.internal.active ? s : { internal: { ...s.internal, active: true } },
    );
    let raf = 0;
    const tick = (now: number) => {
      raf = requestAnimationFrame(tick);
      r3fAdvance(now, true, rootStore.getState());
    };
    raf = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(raf);
  }, [setFrameloop, rootStore]);
  ...
  useEffect(() => {
    const apply = () => {
      ...
      // di: the app's default theme is light/cream, and the shader ramp is
      // dark-first (white base washes out on light backgrounds), so invert
      // on light theme instead of dark.
      (circleRef.current.material.uniforms.uInverted as { value: number }).value = isDark ? 0 : 1;
    };
  ...
  useFrame((_, delta: number) => {
    ...
    (u.uTime as { value: number }).value += delta * 0.5;

    const uOpacity = u.uOpacity as { value: number };
    if (uOpacity.value < 1) {
    ...
    if (modeRef.current === "manual") {
      ...
      const t = (u.uTime as { value: number }).value * 2;
    ...
    (u.uAnimation as { value: number }).value += delta * animSpeedRef.current;
    (u.uInputVolume as { value: number }).value = curInRef.current;
    (u.uOutputVolume as { value: number }).value = curOutRef.current;
    (u.uColor1 as { value: { lerp: (c: unknown, a: number) => void } }).value.lerp(
      targetColor1Ref.current,
      0.08,
    );
    (u.uColor2 as { value: { lerp: (c: unknown, a: number) => void } }).value.lerp(
  ...
  useEffect(() => {
    ...
    // di fix: r3f's own context-restored handling does not resume rendering
    // after the StrictMode teardown, so force three.js to rebuild its state.
    const kick = () => {
      gl.forceContextRestore();
    };
    canvas.addEventListener("webglcontextlost", onContextLost, false);
    canvas.addEventListener("webglcontextrestored", kick, false);
    return () => {
      canvas.removeEventListener("webglcontextlost", onContextLost, false);
      canvas.removeEventListener("webglcontextrestored", kick, false);
    };
  }, [gl, setFrameloop, rootStore]);
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      ...
      uInverted: new THREE.Uniform(isDark ? 0 : 1),
